...
def extract_feature(dataset, model, output_dir, idx_start, idx_end):
    ...
    for i in tqdm(list(range(idx_start + mpu.get_expert_data_parallel_rank(), idx_end, mpu.get_expert_data_parallel_world_size()))):
...
if __name__ == "__main__":
    ...
    train_ds = SFTDataset(5000, data_path[0], **kwargs)
    #sft_dataset = SFTDataset(args.num_samples, None, **kwargs)


    extract_feature(train_ds, unwrapped_model, os.path.join(args.output_dir, "train"), 0, int(args.num_samples * 0.98))
    extract_feature(train_ids, unwrapped_model, os.path.join(args.output_dir, "valid"), int(args.num_samples * 0.98), int(args.num_samples * 0.99))
    extract_feature(train_ids, unwrapped_model, os.path.join(args.output_dir, "test"), int(args.num_samples * 0.99), args.num_samples)

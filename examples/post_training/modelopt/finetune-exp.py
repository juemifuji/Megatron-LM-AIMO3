...
class SFTDataset(torch.utils.data.Dataset):
    ...
    def _process_and_pack_example(self):
        ...
        # 如果 sample 已经超过 required length，直接 truncate
        ...
        # 否则 PAD 到 required 长度
        pad_len = required_packed_tokens - token_count

        # 用 -100 填充 input_ids
        padded_input_ids = input_ids + [-100] * pad_len
    ...
    def _process_example(self, example: Dict[str, Any]):
        ...
        # input_ids = self.tokenizer.apply_chat_template(example, reasoning_effort="high") # for gpt-oss
        input_ids = self.tokenizer(example[1]["content"])["input_ids"]
...
def forward_step(data_iterator, model: GPTModel):
    ...
    #print("=======================================get_batch(data_iterator)================", batch)
    #tokens = batch["tokens"]
    #labels = batch["labels"]
